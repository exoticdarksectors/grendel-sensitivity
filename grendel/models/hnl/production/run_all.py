...
from grendel.models.hnl.mass_grid import MASS_GRID

def _worker_meson(flavor: str, channel: str, masses: list, n_pool: int, seed: int) -> str:
    from grendel.models.hnl.production.generate_meson_csvs import (
...
def _worker_induced_tau(flavor: str, masses: list, n_pool: int, seed: int) -> str:
    from grendel.models.hnl.production.generate_induced_tau import build_tau_pool, process_flavor
    rng = np.random.default_rng(seed)
    random.seed(seed)
...
def _worker_kaon(flavor: str, masses: list, n_pool: int, seed: int) -> str:
    from grendel.models.hnl.production.generate_kaon_csvs import sample_kaon_4vectors, process_flavor
    rng = np.random.default_rng(seed)
    random.seed(seed)
...
def _worker_baryon(flavor, masses, n_pool, seed):
    from grendel.models.hnl.production.generate_baryon_csvs import build_lambda_b_pool, process_flavor
...
def _worker_prompt_tau_stage2(flavor: str, masses: list, seed: int) -> str:
    from grendel.models.hnl.production.madgraph.run_tau_production import _load_pool, process_flavor
    rng = np.random.default_rng(seed)
    random.seed(seed)
...
def _worker_wz(flavor: str, masses: list, n_events: int, nb_core: int) -> str:
    from grendel.models.hnl.production.madgraph.run_wz_production import run_single_point
    failed = []
    for m in masses:
...
def main():
    ...
    from grendel.models.hnl.production.paths import describe_paths
    print("HNL artifact paths:")
    print(describe_paths())

    if "prompt_tau" in selected:
        from grendel.models.hnl.production.madgraph.run_tau_production import POOL_CSV, generate_tau_pool
        from grendel.models.hnl.production.paths import existing_tau_pool_csv
        from grendel.models.hnl.mass_grid import N_EVENTS_DEFAULT
    ...
    if not args.skip_combine:
        print("\nCombining channels...")
        from grendel.models.hnl.production.combine_channels import combine_for_point
