...
from grendel.models.hnl.mass_grid import MASS_GRID, N_EVENTS_DEFAULT
from grendel.production.constants import K_FACTOR_EW_BY_PROCESS, FLAVOR_TO_MG5
from grendel.models.hnl.production.madgraph._mg5_common import MG5_EXE, LHAPDF_CONFIG
from grendel.models.hnl.production.madgraph.runner import (
    ensure_process_dir, run_events as mg5_run_events, write_run_card,
)
from grendel.models.hnl.production.paths import llp_csv_path, scale_weight_column
from grendel.models.hnl.production.paths import MG5_WORK_DIR

from grendel.io.paths import third_party_dir

MODEL_DIR = third_party_dir() / "SM_HeavyN_CKM_AllMasses_LO"
...
def convert_lhe(lhe_path, csv_path):
    from grendel.models.hnl.production.madgraph.lhe_to_csv import LHEParser
...
def main():
    ...
    if not MG5_EXE.exists():
        ...
        print("Set $GRENDEL_MG5_EXE or fetch MadGraph with third_party/fetch.py.")
        return 1
    ...
    if not LHAPDF_CONFIG.exists():
        ...
        print("Set $GRENDEL_LHAPDF_CONFIG to a working lhapdf-config binary "
