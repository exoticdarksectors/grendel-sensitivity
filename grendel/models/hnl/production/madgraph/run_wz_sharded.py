...
from grendel.models.hnl.production.paths import CACHE_DIR, TMP_DIR
...
def _run_shard(payload: tuple) -> tuple[int, int, Path, float]:
    ...
    env = os.environ.copy()
    env["GRENDEL_HNL_MG5_WORK_DIR"] = str(shard_work)
    env["PYTHONUNBUFFERED"] = "1"

    t0 = time.time()
    with log_path.open("w") as log:
        log.write(f"CMD: {' '.join(cmd)}\n")
        log.write(f"GRENDEL_HNL_MG5_WORK_DIR={shard_work}\n\n")
        log.flush()
