...
This is the ROOT+Pythia8 stage of the acceptance pipeline: it must run under
a Python that has PyROOT + Pythia8 (``ROOT.TPythia8``) and FairShip fetched
by ``third_party/fetch.py``. The downstream analysis consumes the cached
templates without ROOT.
...
charged stable daughters.
    ...
    python -m grendel.models.hnl.templates.fairship --flavor Ue Umu Utau --n-templates 20000
"""
...
from . import fairship_decay as fd  # needs PyROOT + Pythia8
...
def main(argv=None):
    ...
    ap.add_argument("--out", type=Path, default=None,
                    help="template directory (default: $GRENDEL_HNL_TEMPLATES_DIR)")
    ...
    out_dir = (args.out or ModelPaths.resolve("hnl").templates).expanduser().resolve()
