"""FairShip-backed HNL lifetime and decay sampling.

Supports all three flavors (Ue, Umu, Utau) via ``FLAVOR_TO_COUPLINGS``. Uses
the FairShip modules ``hnl.py``, ``readDecayTable.py``, ``shipunit.py`` and
``pythia8_conf_utils.py`` from the checkout made by ``third_party/fetch.py``;
a full FairShip build is not required, but PyROOT with Pythia8 is.
"""
...
HNL_PDG = 9900015
DEFAULT_DECAY_CONFIG = Path(__file__).resolve().parents[1] / "data" / "fairship_decay_selection.conf"
...
def fairship_python_dir() -> Path:
    """FairShip's ``python/`` directory in the third-party checkout."""
    return third_party_dir() / "FairShip" / "python"


def _ensure_fairship_paths():
    """Put FairShip's python modules on sys.path and set the FAIRSHIP env."""
    root = third_party_dir() / "FairShip"
    os.environ.setdefault("FAIRSHIP", str(root))
    python_dir = str(fairship_python_dir())
...
def _require_root():
    ...
    if root is None:
        ...
        raise RuntimeError(
            ...
            "Run under an environment with PyROOT and Pythia8 available."
        ) from _ROOT_ERROR
